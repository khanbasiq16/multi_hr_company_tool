...
// Helper function for dynamic currency conversion
...
export async function POST(req) {
    try {
        ...
        const transferfromid = uuidv4()
        const updatedFromBank = {
            ...frombank.data(),
            Transferlogs: arrayUnion({
                transferid:transferfromid,
                status: "Debit",
                amount: `${amount} ${fromsymbole}`,
                convertedAmount: `${convertedAmount} ${toSymbol}`,
                rate: `${fromRate} ${fromsymbole}`,
            ...
            balance: parseFloat(frombank.data().balance) - parseFloat(amount),
        };

        // ===================== UPDATE TO BANK (CREDIT) =====================
         const transfertoid = uuidv4()
        const updatedToBank = {
            ...tobank.data(),
            Transferlogs: arrayUnion({
                transferid:transfertoid,
                status: "Credit",
                amount: `${amount} ${fromsymbole}`,
                convertedAmount: `${convertedAmount} ${toSymbol}`,
                rate: `${fromRate} ${fromsymbole}`,
        ...
        return NextResponse.json({
            success: true,
            message: `${amount} Debited from ${frombank?.data()?.banktitle} and Credited to ${tobank?.data()?.banktitle}`,
            convertedAmount,
