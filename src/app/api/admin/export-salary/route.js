...
export async function POST() {
    try {
        ...
        const salaryRows = snapshot.docs.map(doc => {
            ...
            // 2. Attendance Stats Counts
            const lates = monthData.filter(a => a.checkin?.status === "Late").length;
            const actualAbsents = monthData.filter(a => a.checkin?.status === "Absent").length;
            const halfDays = monthData.filter(a => a.checkin?.status === "Half Day").length;
            const shortDays = monthData.filter(a => a.checkin?.status === "Short Day").length;
            const earlyCheckOuts = monthData.filter(a => a.checkout?.status === "Early Check Out").length;

            // 3. Conversion Logic (Deduction Rules)
            // Rules: 3 Late = 1 Absent, 2 Half Day = 1 Absent, 3 Short Day = 1 Absent, 2 Early = 1 Absent
            const convertedFromLates = Math.floor(lates / 3);
            const convertedFromHalfDays = Math.floor(halfDays / 2);
            const convertedFromShortDays = Math.floor(shortDays / 3);
            const convertedFromEarlyOuts = Math.floor(earlyCheckOuts / 2);

            const totalAbsentsForDeduction = actualAbsents + convertedFromLates + convertedFromHalfDays + convertedFromShortDays + convertedFromEarlyOuts;

            // 4. Financial Calculations
            const perDaySalary = Math.round(basicSalary / 30);
            const monthlyTax = calculateTax(basicSalary);

            const totalDeductionAmount = totalAbsentsForDeduction * perDaySalary;
            ...
            const grossTotal = (basicSalary + allowances + bonus + commission) - (monthlyTax + totalDeductionAmount + advance);
            ...
            return {
                ...
                "Early Outs": earlyCheckOuts,
                "Total Absents": totalAbsentsForDeduction,
                "Tax": monthlyTax,
                "Advance": advance,
                "Net Payable": grossTotal
            };
        });
