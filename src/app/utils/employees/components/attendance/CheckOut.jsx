...
const CheckOut = () => {
  ...
  // ✅ Convert ISO → readable 12-hour time
  const isoTo12Hour = (isoString) => {
    const date = new Date(isoString);
    let hours = date.getHours();
    const minutes = date.getMinutes().toString().padStart(2, "0");
  ...
  // ✅ Pakistan timezone accurate Date object
  const getPakistanDateObject = () => {
    const now = new Date();
    const localTime = now.getTime();
    const localOffset = now.getTimezoneOffset() * 60000;
    const utc = localTime + localOffset;
    const pakistanOffset = 5 * 60 * 60000; // UTC+5
    return new Date(utc + pakistanOffset);
  };

  // ✅ Get formatted Pakistan time
  ...
  // ✅ Handle check-out time window (Pakistan-based)
  useEffect(() => {
    if (!user?.department?.checkOutTime) return;

    const checkOutStr = user.department.checkOutTime;
    const officeCheckOut = getPakistanDateObject();
    ...
    const checkWindow = () => {
      const now = getPakistanDateObject();
      const canCheck = now >= enableTime && now <= disableTime;
      setCanCheckOut(canCheck);
    };
  ...
  // ✅ Normal Check-out (Pakistan Time)
  const normalcheckout = async () => {
    try {
      setLoadingsubmit(true);
      const ip = await getcurrentip();
  ...
  // ✅ Late/Early Check-out (Pakistan Time)
  const latecheckout = async () => {
    try {
      setLoadingsubmit(true);
      const ip = await getcurrentip();
  ...
  // ✅ Work Summary Submission (Pakistan Time)
  const submitWorkSummary = async () => {
    try {
      setLoadingsubmit(true);
      const ip = await getcurrentip();
        ...
        <DialogContent className="h-screen overflow-y-auto rounded-2xl p-6 bg-white dark:bg-gray-900 dark:text-gray-100">
          ...
          <div className="space-y-4 mt-4 p-2">
            ...
            <div className="text-sm text-gray-500 mt-4 border-t pt-3">
              ...
              <p>Date: {getPakistanDateObject().toLocaleDateString("en-PK")}</p>
            </div>
          </div>
