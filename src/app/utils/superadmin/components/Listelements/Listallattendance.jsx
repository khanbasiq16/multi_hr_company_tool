...
const colorMap = {
  ...
  "Short Day": "bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300", // proper purple
...
const ListAllAttendance = () => {
  ...
  const filteredAttendance = useMemo(() => {
    ...
    return list.map(att => ({
      ...
      if (selectedStatus && selectedStatus !== "All Status") {
        const statusToCheck = activeTab === "checkin" ? att.checkin.status : att.checkout.status;
  ...
  /* Total Deduction based on percentage (only for Check-In) */
  const totalDeduction = useMemo(() => {
    if (!selectedEmployee || activeTab !== "checkin") return 0;
    const selectedEmp = employees.find(emp => emp.employeeName === selectedEmployee);
  ...
  { 
    ...
    header: "Duration", 
    cell: ({ row }) => row.original[activeTab === "checkin" ? "stopwatchTime" : "checkoutStopwatchTime"]
  },
    ...
    <div className="bg-white dark:bg-gray-950 p-6 rounded-2xl shadow-md h-[85vh] overflow-auto">
      ...
      {selectedEmployee && activeTab === "checkin" && (
        <div className="flex gap-6 mb-4 text-sm font-medium">
          <span>Total Salary: <strong>{totalSalary} PKR</strong></span>
          <span>Absent Days: <strong>{totalAbsentDays}</strong></span>
          <span>Deduction: <strong className="text-red-600">{totalDeduction} PKR</strong></span>
          <span>Net Salary: <strong className="text-green-600">{netSalary} PKR</strong></span>
        </div>
      )}
        ...
        <div className="mb-4 flex flex-wrap gap-4 text-sm font-medium text-gray-700 dark:text-gray-300">
          {Object.keys(statusCounts).map(status => (
            <span key={status} className="flex items-center gap-1">
              <StatusBadge status={status} /> {statusCounts[status]} day{statusCounts[status] !== 1 ? "s" : ""}
            </span>
          ))}
        </div>
